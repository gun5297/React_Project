...
const Watch = () => {
  ...
  // 유저가 선택한 동영상 찾기
  const thisMovie = allMovies.find(
    (movie) => movie.movie_id === Number(Movie_ID)
  );
  const channelData = thisMovie ? Channel[thisMovie.movie_channel] : null;
    ...
    <WatchWrap>
      <div className="video-wrap">
        ...
        <Below
          ...
          channelName={channelData ? channelData.channel_name : "알 수 없음"}
          channelSubscribers={
            channelData
              ? formatSubscribers(channelData.channel_subscribers)
              : "알 수 없음"
          }
          movieLikeCount={formatLikes(thisMovie.movie_like_count)}
          channelImage={channelData ? channelData.channel_image : ""}
        />
      </div>
      <WatchListWrap>
        <WatchList
